# 文件地图（由 make_ai_handoff.py 自动生成，勿手改）

> 生成时间：2026-09-24 17:11；共 281 个条目；体积单位 KB/MB。

## 提交物（5 项，43.2 MB）

| 文件 | 大小 | 用途 |
|---|---|---|
| `更新世界的锋芒_SoundInsight_复赛作品.zip` | 42.4 MB |  |
| `更新世界的锋芒_SoundInsight_其他材料.zip` | 463.1 KB |  |
| `更新世界的锋芒_SoundInsight_复赛作品.pdf` | 210.1 KB |  |
| `更新世界的锋芒_SoundInsight_Demo.zip` | 108.0 KB |  |
| `更新世界的锋芒_SoundInsight_复赛作品.docx` | 48.0 KB |  |

## 主文档与模板（10 项，0.1 MB）

| 文件 | 大小 | 用途 |
|---|---|---|
| `hackathon-决赛入围定稿作品提交模板-天池版.docx` | 35.8 KB |  |
| `hackathon-复赛作品提交模板-天池版.docx` | 35.7 KB | 官方复赛模板原文（格式对照基准） |
| `competition_v4.md` | 20.1 KB | 主文档（13 章，含模板九章 + 合规披露 + 局限） |
| `competition_v3.txt` | 8.8 KB | 官方模板九章版主文档 |
| `README.md` | 8.2 KB |  |
| `competition_v2.md` | 7.0 KB | 初赛版文档 |
| `competition_v2.txt` | 7.0 KB |  |
| `DEPLOY_GUIDE.md` | 4.2 KB |  |
| `deployment/README.md` | 0.6 KB |  |
| `REPO_INTRO.txt` | 0.6 KB |  |

## 验证报告（23 项，0.1 MB）

| 文件 | 大小 | 用途 |
|---|---|---|
| `error_taxonomy.md` | 8.2 KB | 错误分类学：FP/FN 构成 + 标注噪声人工终审 |
| `number_audit.md` | 8.0 KB | 数字一致性审计结果（含官方模板格式对照节） |
| `llm_baseline.md` | 6.4 KB | LLM 对照报告（deepseek + qwen3.7-plus，含循环性红利声明） |
| `edge_case_benchmark.py` | 5.6 KB |  |
| `qna_preparation.md` | 5.0 KB | 答辩 Q&A（10 问，含口径警示） |
| `results_summary.py` | 4.8 KB |  |
| `results_summary.md` | 4.3 KB | 冻结数字唯一权威源（含 ±1 混淆矩阵调和行） |
| `length_bucket_eval.py` | 3.9 KB |  |
| `MODEL_CARD.md` | 3.4 KB | 模型卡：任务/基座/指标/六条局限 |
| `edge_cases.md` | 3.2 KB |  |
| `calibration_eval.py` | 3.0 KB |  |
| `check_edge_cases.py` | 2.7 KB |  |
| `edge_case_benchmark.md` | 2.6 KB | 六类边界场景定向探针 |
| `user_scenarios.md` | 2.6 KB |  |
| `ablation_summary.py` | 2.2 KB |  |
| `action_report_template.md` | 2.1 KB |  |
| `stats_validation.md` | 1.5 KB | 统计验证（PR 曲线 / 学习曲线） |
| `length_bucket_eval.md` | 1.4 KB | tokenizer 长度分桶（>128 token 截断桶 F1 0.565） |
| `calibration_eval.md` | 1.2 KB | 校准评估：ECE 0.0122 与决策区间过度自信 |
| `throughput_eval.md` | 0.9 KB | GPU/CPU 吞吐实测 |
| `ablation_summary.md` | 0.8 KB | 消融实验 3 组结论 |
| `confidence_tiered.md` | 0.7 KB | 三星补漏置信度分层与中置信剔除记录 |
| `significance_test.md` | 0.6 KB | Welch t 检验（p=0.000932） |

## 过程与交接文档（17 项，0.2 MB）

| 文件 | 大小 | 用途 |
|---|---|---|
| `docs/retrospective_and_reflection.md` | 27.9 KB |  |
| `docs/project_full_record.md` | 21.6 KB | 初赛→复赛全流程记录 |
| `docs/D13_seal_declaration.md` | 19.7 KB | D13 封包声明（提交技术质检终审） |
| `PROGRESS_SYNC.md` | 15.2 KB |  |
| `docs/file_inventory.md` | 12.3 KB | 项目文件总索引 |
| `PROJECT_BRIEF_QWEN.md` | 10.7 KB |  |
| `QWEN_HANDOFF.md` | 10.7 KB |  |
| `docs/process_review_d10.md` | 8.5 KB |  |
| `docs/legacy_materials_notice.md` | 8.3 KB |  |
| `docs/finals_stage.md` | 7.1 KB |  |
| `docs/work_summary_d7.md` | 6.2 KB |  |
| `docs/repo_hygiene_scan.md` | 3.7 KB |  |
| `docs/dataset_audit.md` | 3.5 KB | 数据集字段考古 |
| `docs/D14_resume_checklist.md` | 3.0 KB | 收尾四步清单（录视频/打包/上传/补推送） |
| `docs/drift_plan.md` | 2.8 KB | 数据漂移监控方案 |
| `AI_INDUCTION_REDLINE.md` | 2.7 KB |  |
| `docs/year_split_output.txt` | 0.8 KB | 年份分桶脚本输出原文 |

## 演示材料（6 项，18.1 MB）

| 文件 | 大小 | 用途 |
|---|---|---|
| `SoundInsight：跨境电商耳机音质差评智能归因系统.pptx` | 18.1 MB |  |
| `ppt_text_dump.md` | 11.4 KB |  |
| `video_script.md` | 5.7 KB | 演示视频脚本（200 秒 / 9 镜头） |
| `video_script_silent.md` | 4.4 KB | 无口播拍摄卡（操作步骤 + 画面要点） |
| `视频素材/素材清单.md` | 2.9 KB |  |
| `video_script.srt` | 2.7 KB | 字幕文件（28 条 / 0:00-3:20 / UTF-8） |

## 图表（19 项，1.6 MB）

| 文件 | 大小 | 用途 |
|---|---|---|
| `视频素材/06_时序趋势.png` | 200.9 KB |  |
| `视频素材/05_系统架构.png` | 180.1 KB |  |
| `视频素材/01_学习曲线.png` | 162.6 KB |  |
| `视频素材/04_校准曲线.png` | 151.6 KB |  |
| `视频素材/03_混淆矩阵.png` | 122.0 KB |  |
| `视频素材/02_PR曲线.png` | 95.6 KB |  |
| `architecture.png` | 82.9 KB |  |
| `trend_over_time.png` | 78.1 KB |  |
| `视频素材/07_结尾卡.png` | 65.3 KB |  |
| `视频素材/00_标题卡.png` | 64.4 KB |  |
| `learning_curve.png` | 60.5 KB |  |
| `calibration_curve.png` | 57.1 KB |  |
| `demo_output.png` | 42.0 KB |  |
| `confusion_matrix.png` | 38.8 KB |  |
| `exp06_最终二分类模型/confusion_matrix.png` | 38.8 KB |  |
| `exp07_ablation_A/confusion_matrix.png` | 38.1 KB |  |
| `exp09_ablation_C/confusion_matrix.png` | 37.8 KB |  |
| `exp08_ablation_B/confusion_matrix.png` | 37.7 KB |  |
| `pr_curve.png` | 37.7 KB |  |

## 证据·标注流程（19 项，3.4 MB）

| 文件 | 大小 | 用途 |
|---|---|---|
| `three_star_input.jsonl` | 1.1 MB |  |
| `review_input.jsonl` | 1.0 MB |  |
| `treble_candidates.jsonl` | 421.3 KB |  |
| `conf_review_input.jsonl` | 274.4 KB |  |
| `neg_review_input.jsonl` | 197.3 KB |  |
| `review_result.jsonl` | 62.1 KB |  |
| `exp03_LLM复核与标签清洗/review_result.jsonl` | 62.1 KB |  |
| `three_star_result.jsonl` | 50.5 KB |  |
| `exp03_LLM复核与标签清洗/three_star_result.jsonl` | 50.5 KB |  |
| `conf_review_result.jsonl` | 30.2 KB |  |
| `human_review_50.csv` | 26.3 KB | 标注人工抽查 50 条（78% 通过） |
| `treble_result.jsonl` | 13.3 KB |  |
| `neg_review_result.jsonl` | 11.7 KB |  |
| `exp03_LLM复核与标签清洗/neg_review_result.jsonl` | 11.7 KB |  |
| `docs/treble_fix_proposal.md` | 9.6 KB |  |
| `human_review_conf30.csv` | 9.4 KB | 中置信 8 条人工复核表（3/8 通过→全剔除） |
| `human_review_noise16.csv` | 7.1 KB | 标注噪声 16 条人工终审表（15/16 确认） |
| `docs/treble_failure_analysis.md` | 6.2 KB |  |
| `prep_review_input.py` | 1.0 KB |  |

## 证据·对照与错误分析（35 项，8.2 MB）

| 文件 | 大小 | 用途 |
|---|---|---|
| `val_preds_dump.csv` | 7.5 MB | val_v2 全量推理快照（prob/pred/长度/标签） |
| `llm_eval_input.jsonl` | 420.2 KB | LLM 对照子集（1000 条 = val_v2 正例全集 + 749 负例） |
| `llm_err_input.jsonl` | 65.2 KB |  |
| `llm_eval_small_preds.jsonl` | 38.9 KB |  |
| `llm_err_classes.jsonl` | 20.8 KB |  |
| `spot_check_30.jsonl` | 15.2 KB |  |
| `llm_eval_out_5shot_1.jsonl` | 6.7 KB |  |
| `llm_eval_out_5shot_2.jsonl` | 6.7 KB |  |
| `llm_eval_out_review_1.jsonl` | 6.7 KB |  |
| `llm_eval_out_review_2.jsonl` | 6.7 KB |  |
| `llm_eval_out_zero_1.jsonl` | 6.7 KB |  |
| `llm_eval_out_zero_2r.jsonl` | 6.7 KB |  |
| `llm_qwen_out_5shot_1.jsonl` | 6.7 KB |  |
| `llm_qwen_out_5shot_2.jsonl` | 6.7 KB |  |
| `llm_qwen_out_review_1.jsonl` | 6.7 KB |  |
| `llm_qwen_out_review_2.jsonl` | 6.7 KB |  |
| `llm_qwen_out_zero_1.jsonl` | 6.7 KB |  |
| `llm_qwen_out_zero_2.jsonl` | 6.7 KB |  |
| `llm_eval_out_5shot_0.jsonl` | 6.6 KB |  |
| `llm_eval_out_review_0.jsonl` | 6.6 KB |  |
| `llm_eval_out_zero_0.jsonl` | 6.6 KB |  |
| `llm_qwen_out_5shot_0.jsonl` | 6.6 KB |  |
| `llm_qwen_out_review_0.jsonl` | 6.6 KB |  |
| `llm_qwen_out_zero_0.jsonl` | 6.6 KB |  |
| `llm_eval_metrics.py` | 4.6 KB |  |
| `llm_qwen_metrics.py` | 3.2 KB |  |
| `llm_eval_out_5shot_3.jsonl` | 2.2 KB |  |
| `llm_eval_out_review_3.jsonl` | 2.2 KB |  |
| `llm_eval_out_zero_3.jsonl` | 2.2 KB |  |
| `llm_qwen_out_5shot_3.jsonl` | 2.2 KB |  |
| `llm_qwen_out_review_3.jsonl` | 2.2 KB |  |
| `llm_qwen_out_zero_3.jsonl` | 2.2 KB |  |
| `llm_eval_metrics.json` | 1.1 KB |  |
| `llm_qwen_metrics.json` | 1.0 KB |  |
| `learning_curve_results.json` | 0.1 KB |  |

## 数据（14 项，405.9 MB）

| 文件 | 大小 | 用途 |
|---|---|---|
| `electronics_prefix.bin` | 128.0 MB |  |
| `labeled_llm.csv` | 38.3 MB | LLM 复核后标签（当前工作集正例 1280） |
| `labeled_llm_before_mid_demote.csv` | 38.3 MB |  |
| `labeled_llm_before_mid_remove.csv` | 38.3 MB |  |
| `labeled_llm_before_treble.csv` | 38.3 MB | 高音补捞前备份（冻结实验口径 1257） |
| `exp03_LLM复核与标签清洗/labeled_llm.csv` | 38.3 MB |  |
| `labeled_expanded.csv` | 37.1 MB |  |
| `electronics_expanded.csv` | 37.1 MB | 主数据集（10 万条，含 timeStamp 恢复） |
| `val_v2.csv` | 7.1 MB | 固定验证集（20000 条 / 251 正例），红线冻结 |
| `labeled_data_final.csv` | 2.1 MB |  |
| `local_data.csv` | 2.1 MB |  |
| `train_quick.csv` | 1.2 MB |  |
| `sample_reviews_100.csv` | 57.9 KB |  |
| `agent_test_input.csv` | 27.9 KB |  |

## 代码（87 项，0.3 MB）

| 文件 | 大小 | 用途 |
|---|---|---|
| `scan_repo_hygiene.py` | 17.7 KB |  |
| `build_pdf_v2.py` | 12.4 KB |  |
| `check_doc_numbers.py` | 12.3 KB | 数字审计 + 模板格式对照脚本 |
| `build_pdf.py` | 11.2 KB |  |
| `make_ai_handoff.py` | 10.3 KB |  |
| `build_submission.py` | 9.3 KB |  |
| `demo_sound_v2.py` | 9.1 KB | 本地 Gradio Demo（三页；已内置代理绕过） |
| `report_builder.py` | 8.3 KB |  |
| `ablation_train.py` | 8.2 KB |  |
| `soundinsight_agent.py` | 8.1 KB | 一键洞察 Agent（md/excel、zh/en、非英文跳过） |
| `pack_final.py` | 7.9 KB | 提交包打包与四项自检 |
| `train_sound_model.py` | 7.8 KB |  |
| `md_to_pdf.py` | 7.6 KB | 主文档 Markdown → PDF（内嵌中文字体） |
| `learning_curve.py` | 6.8 KB |  |
| `diagnose_treble.py` | 6.6 KB |  |
| `fetch_electronics.py` | 6.5 KB |  |
| `distilbert_cv.py` | 5.4 KB |  |
| `md_to_docx.py` | 5.4 KB | 主文档 Markdown → Word |
| `train_final.py` | 5.2 KB |  |
| `train_roberta_quick.py` | 5.2 KB |  |
| `predict_core.py` | 5.1 KB | 共享推理核心（含非英文显式拒绝） |
| `fix_ppt_threshold.py` | 4.8 KB |  |
| `make_video_assets.py` | 4.5 KB |  |
| `make_samples.py` | 4.4 KB |  |
| `retime_srt.py` | 4.2 KB |  |
| `tune_per_class_threshold.py` | 4.0 KB |  |
| `upload_models.py` | 3.9 KB |  |
| `test_model.py` | 3.9 KB |  |
| `merge_refine.py` | 3.8 KB |  |
| `train_multilabel.py` | 3.8 KB |  |
| `distilbert_vs_llm.py` | 3.6 KB |  |
| `extend_data.py` | 3.6 KB |  |
| `baseline_cv.py` | 3.5 KB |  |
| `finalize_curve.py` | 3.3 KB |  |
| `audit_ppt.py` | 3.0 KB |  |
| `deploy_check.py` | 3.0 KB | 在线 Demo 单条验收脚本 |
| `svm_ttest.py` | 2.9 KB |  |
| `a4_mid_remove.py` | 2.8 KB |  |
| `fix_ppt_roles.py` | 2.7 KB |  |
| `year_split_eval.py` | 2.7 KB |  |
| `prep_llm_eval.py` | 2.7 KB |  |
| `capture_demo_output.py` | 2.6 KB |  |
| `apply_mid_demotion.py` | 2.6 KB |  |
| `rebuild_video_timing.py` | 2.5 KB |  |
| `download_models.py` | 2.5 KB |  |
| `retier_conf.py` | 2.4 KB |  |
| `arch_diagram.py` | 2.3 KB |  |
| `val_pred_dump.py` | 2.3 KB |  |
| `label_v3.py` | 2.3 KB |  |
| `trend_over_time.py` | 2.3 KB |  |
| `pr_curve.py` | 2.3 KB |  |
| `label_final.py` | 2.3 KB |  |
| `ppt_speed_fix.py` | 2.3 KB |  |
| `restore_timestamps.py` | 2.3 KB |  |
| `throughput_bench.py` | 2.2 KB |  |
| `verify_online_report.py` | 2.2 KB |  |
| `check_video.py` | 2.1 KB |  |
| `demo_sound.py` | 2.1 KB |  |
| `batch_check.py` | 2.1 KB | 在线 Demo 批量验收脚本 |
| `taxonomy_agg.py` | 2.0 KB |  |
| `prep_refine.py` | 2.0 KB |  |
| `merge_review.py` | 2.0 KB |  |
| `pack_when_free.py` | 1.9 KB |  |
| `archive_exp.py` | 1.9 KB |  |
| `api_server.py` | 1.8 KB | FastAPI 服务（/health、/predict） |
| `miss_rate.py` | 1.7 KB |  |
| `install.bat` | 1.7 KB |  |
| `prep_quick_split.py` | 1.6 KB |  |
| `verify_download_urls.py` | 1.5 KB |  |
| `extract_template.py` | 1.5 KB |  |
| `merge_three_star.py` | 1.4 KB |  |
| `check_studio_build.py` | 1.3 KB |  |
| `prep_err_taxonomy.py` | 1.2 KB |  |
| `verify_docx.py` | 1.2 KB |  |
| `prep_neg_review.py` | 1.1 KB |  |
| `final_acceptance.py` | 1.1 KB |  |
| `hashes.py` | 1.0 KB |  |
| `prep_three_star.py` | 0.9 KB |  |
| `prep_human_review.py` | 0.9 KB |  |
| `启动Demo.bat` | 0.9 KB | 双击启动本地 Demo |
| `text_utils.py` | 0.9 KB |  |
| `exp08_ablation_B/config.json` | 0.5 KB |  |
| `exp09_ablation_C/config.json` | 0.5 KB |  |
| `exp07_ablation_A/config.json` | 0.5 KB |  |
| `打包提交包.bat` | 0.4 KB | 双击执行最终打包与自检 |
| `config.json` | 0.3 KB |  |
| `requirements.txt` | 0.2 KB |  |

## 部署包（7 项，0.0 MB）

| 文件 | 大小 | 用途 |
|---|---|---|
| `deployment/app.py` | 11.3 KB |  |
| `deployment/report_builder.py` | 8.3 KB |  |
| `deployment/.gitattributes` | 2.1 KB |  |
| `deployment/text_utils.py` | 0.9 KB |  |
| `deployment/README_Space.md` | 0.7 KB |  |
| `deployment/requirements.txt` | 0.5 KB |  |
| `deployment/config.json` | 0.2 KB |  |

## 历史材料·初赛（9 项，2.2 MB）

| 文件 | 大小 | 用途 |
|---|---|---|
| `SoundInsight_创意方案/SoundInsight_创意方案.pdf` | 415.2 KB |  |
| `SoundInsight_创意方案.zip` | 361.1 KB |  |
| `SoundInsight_创意方案_v3.pdf` | 351.9 KB |  |
| `SoundInsight_创意方案_v4.pdf` | 347.0 KB |  |
| `SoundInsight_创意方案_v5.pdf` | 341.9 KB |  |
| `SoundInsight_创意方案_v2.pdf` | 328.2 KB |  |
| `SoundInsight_创意方案/demo_output.png` | 41.8 KB |  |
| `SoundInsight_创意方案/confusion_matrix.png` | 32.4 KB |  |
| `SoundInsight_创意方案/training_output.txt` | 0.8 KB |  |

## 参考材料（1 项，0.0 MB）

| 文件 | 大小 | 用途 |
|---|---|---|
| `AI+跨境黑客松巅峰赛 · 复赛公示名单（手机版）.html` | 42.1 KB |  |

## 生成产物·日志（19 项，0.1 MB）

| 文件 | 大小 | 用途 |
|---|---|---|
| `exp01_弱标注交叉验证/distilbert_cv.log` | 20.4 KB |  |
| `exp02_清洗标签交叉验证/distilbert_cv_clean.log` | 10.9 KB |  |
| `insight_report.xlsx` | 7.1 KB |  |
| `exp05_教师一致性/vs_llm.log` | 6.1 KB |  |
| `exp06_最终二分类模型/train_final.log` | 3.4 KB |  |
| `exp04_多标签归因/multilabel.log` | 3.2 KB |  |
| `summary_log.txt` | 3.1 KB |  |
| `batch_report.md` | 2.6 KB |  |
| `insight_report_v2.md` | 2.6 KB |  |
| `insight_report_v2_en.md` | 2.4 KB |  |
| `exp07_ablation_A/training_output.txt` | 1.9 KB |  |
| `training_output.txt` | 0.9 KB |  |
| `exp06_最终二分类模型/training_output.txt` | 0.9 KB |  |
| `batch_report.txt` | 0.8 KB |  |
| `exp02_清洗标签交叉验证/baseline_cv_clean.log` | 0.8 KB |  |
| `exp01_弱标注交叉验证/baseline_cv.log` | 0.6 KB |  |
| `exp08_ablation_B/training_output.txt` | 0.5 KB |  |
| `exp09_ablation_C/training_output.txt` | 0.5 KB |  |
| `capture_log.txt` | 0.5 KB |  |

## 大文件·不入库（4 项，1246.7 MB）

| 文件 | 大小 | 用途 |
|---|---|---|
| `roberta-base/` | 478.3 MB | RoBERTa 探测用基座缓存 |
| `distilbert-base-uncased/` | 256.2 MB | 基座模型缓存（HF/ModelScope） |
| `multi_label_model/` | 256.1 MB | 冻结的多标签归因模型权重（约 256MB），不入 git |
| `sound_model/` | 256.1 MB | 冻结的二分类模型权重（约 256MB），不入 git |

## 其他（6 项，64.2 MB）

| 文件 | 大小 | 用途 |
|---|---|---|
| `更新世界的锋芒_SoundInsight_演示视频.mp4` | 46.1 MB |  |
| `SoundInsight：跨境电商耳机音质差评智能归因系统.pptx.bak` | 18.1 MB |  |
| `视频录制速查.md` | 3.1 KB |  |
| `hashes.txt` | 0.9 KB |  |
| `per_class_thresholds_probe.json` | 0.4 KB |  |
| `.gitignore` | 0.3 KB |  |
