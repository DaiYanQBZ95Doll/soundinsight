...
- **决赛模板已到手（2026-09-24）**：`hackathon-决赛入围定稿作品提交模板-天池版.docx`（官方，36,689 B，已入仓库）。与复赛的**实质差别**：① 主文档＝**填写后的该模板**（.docx/.pdf），不是自拟的 v4 文档 → v4 内容需按模板九节重排；② zip 与主文档命名改为 `团队名_方案名称_决赛入围定稿作品`；③ 模板 5.2"阿里云百炼模型"表须填**真实调用**：qwen3.7-plus + 阿里云百炼 Token Plan API（跨 LLM 对照，评审用途非产品推理），**不是"未使用"**（该表述属 P0-1 路线 (a) 过时状态，9/5 路线 (b) 后作废；详见 `docs/v2_acceptance_benchmark.md` §6.0）；④ 视频建议 3–5 分钟（现有 3:23 可复用）；⑤ 明确"代码仓库不强制公开源码，可私有仓库 + 授权评审可见，或用 Demo + 视频验证"。逐条要求、差别与交付物映射见 `docs/finals_stage.md` §三/§五；`extract_template.py` 已支持传入模板路径，便于后续"填写稿 vs 模板"格式对照。
- **勘误纪律（2026-09-24）**：本节上方 P0-1 路线 (a) 的记录（"百炼栏写未使用"）是当日真实状态，**已被 9/5 路线 (b) 取代**，不得作为现状引用——DSH 曾两次据此误写决赛 5.2 填法，故在此显式标注。
- **审计工具链升级（2026-09-24，按质检方 `docs/v2_acceptance_benchmark.md` §九 要求）**：`check_doc_numbers.py` 由"单项检查"扩为**代际感知**的五节新检查——① 口径配对检查扩展到**全指标、按阈值档**（一行并排两档却未标阈值即 FAIL）；② **代际混用**检查（`[v1]`/`[v2]` 标签语法；v2 tokens 表一填就自动生效）；③ **提交包冻结校验**（红线 9：磁盘与 `hashes.txt` 双向核对已提交的复赛包哈希）；④ **决赛模板合规**（九节齐备 + 编号唯一 + 在线链接表 + 团队信息 + **5.2 百炼栏按既成事实填写**，假称"未使用"即 FAIL）；⑤ **v2 可核验产物**（`v2/v2_artifacts.json` 登记模型 SHA256 / threshold.json / 训练命令 / 评估原始输出 / v2 MODEL_CARD，哈希不符即 FAIL）。审计结果：**0 FAIL / 167 PASS**（模板与 v2 两项当前为 SKIP：产物尚未生成，检查项已就绪）。
- **审计自测（负向测试）**：新增 `test_audit_checks.py`（11 项，无需 pytest：`python test_audit_checks.py`）证明每个新检查**会拒绝坏输入**——并排未标阈值、代际混用无标签、包被覆盖、模板假称百炼未使用、v2 哈希不符均判 FAIL；同时验证合法输入不误报。全部 11 项通过。
