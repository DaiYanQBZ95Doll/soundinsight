...
- D1（9.1）：数据扩充至 10 万条、RLCA 两阶段标注产出 1257 正例、最终模型 F1 0.687、基线对比、Agent/Demo/文档草稿、技术底座冻结。
- D2（9.2）：消融实验 3 组、统计验证（PR 曲线 / t-test / 学习曲线）、数据提纯（置信度分层、高音补捞 84→124）、v3 初稿；暴露 P0 问题清单。
- D3（9.3）：修复 7 个 P0 + 4 个 P1（白皮书/98% 清理、下载上传链路、样例集 rating 回填、审计脚本、PPT 分工、高音数字拆解等）、competition_v4.md 完成、团队信息填写。
- D4：仓库中无 D4 独立记录（工作并入 D3/D5 记录，如实注明）。
- D5：五个批次收尾（部署包备齐 / 审计完成 / v4 全 PASS / Demo.zip 与复赛 zip 骨架 / 样例集、视频脚本、反馈模板）。
- D6（D6-D8 遗憾消除清单 A-G）：A 数字硬伤清除（→1280）；B LLM 对照（deepseek 三设置）+ 口径声明修正；C 长度分桶 tokenizer 重做、边界探针、校准 ECE、错误分类学；D 时间戳 100% 恢复、趋势图、年份分桶；E HTTP API、install.bat、吞吐实测、MODEL_CARD、双语报告、非英文拒绝；F 合规披露、漂移计划；G 审计 135 项 PASS、打包、创空间上线并发布、deploy_check、qwen3.7-plus 路线 b、标注噪声人工终审。
- D7：工作总结归档、git push 命令交付、剩余四件用户侧提交物（PPT 精简 / 视频 / 反馈 / 最终 PDF）。
...
- 用户侧待办：GitHub push（已完成）、演示视频录制、最终 PDF 放入复赛作品 zip、（可选）Token Plan key 轮换、human_review_conf30 确认。PPT 已决定保留 20 页不精简（已知偏差，如实记录）。
- 用户验证：本轮不做真实用户反馈收集（已从清单移除，v4 §9.1 第 7 条如实注明"未开展用户验证"）。
